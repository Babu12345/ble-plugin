...
impl From<i32> for MessageTypeId {
    fn from(i: i32) -> Self {
        match i {
            ...
            12 => MessageTypeId::TypeHostCommandQueryConnections,
            ...
            133 => MessageTypeId::TypePluginOnConnectResponse,
            _ => Self::default(),
        }
    }
}
...
impl<'a> From<&'a str> for MessageTypeId {
    fn from(s: &'a str) -> Self {
        match s {
            ...
            "TypeHostCommandQueryConnections" => MessageTypeId::TypeHostCommandQueryConnections,
            ...
            "TypePluginOnConnectResponse" => MessageTypeId::TypePluginOnConnectResponse,
            _ => Self::default(),
        }
    }
}
...
impl<'a> MessageRead<'a> for HostCommandQueryConnections {
    fn from_reader(r: &mut BytesReader, _: &[u8]) -> Result<Self> {
        r.read_to_end();
        Ok(Self::default())
    }
...
impl MessageWrite for HostCommandQueryConnections { }
...
impl<'a> MessageRead<'a> for PluginOnConnectResponse {
    fn from_reader(r: &mut BytesReader, bytes: &'a [u8]) -> Result<Self> {
        let mut msg = Self::default();
        while !r.is_eof() {
            match r.next_tag(bytes) {
...
impl MessageWrite for PluginOnConnectResponse {
    fn get_size(&self) -> usize {
        0
        + if self.address.is_empty() { 0 } else { 1 + sizeof_len((&self.address).len()) }
        + if self.address_type == protocol::BluetoothAddressType::BluetoothAddressTypeUnspecified { 0 } else { 1 + sizeof_varint(*(&self.address_type) as u64) }
    ...
    fn from_reader(r: &mut BytesReader, bytes: &'a [u8]) -> Result<Self> {
        ...
        while !r.is_eof() {
            match r.next_tag(bytes) {
                ...
                Ok(16) => msg.save_on_disconnect = r.read_bool(bytes)?,
                Ok(t) => { r.read_unknown(bytes, t)?; }
                Err(e) => return Err(e),
            }
        }
...
impl MessageWrite for HostCommandConfigureProfile {
    fn get_size(&self) -> usize {
        ...
        + if self.save_on_disconnect == false { 0 } else { 1 + sizeof_varint(*(&self.save_on_disconnect) as u64) }
    }

    fn write_message<W: WriterBackend>(&self, w: &mut Writer<W>) -> Result<()> {
        if self.profile != protocol::BleProfile::BleProfileUnspecified { w.write_with_tag(8, |w| w.write_enum(*&self.profile as i32))?; }
        if self.save_on_disconnect != false { w.write_with_tag(16, |w| w.write_bool(*&self.save_on_disconnect))?; }
        Ok(())
    }
}
