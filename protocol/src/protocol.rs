// This file is @generated by prost-build.
/// Host command. Configure peripheral
/// Random Bluetooth addresses must follow specific MSB bit patterns:
/// - Static Random: MSB bits = 11 (0xC0-0xFF), remaining 46 bits must have at least one 0 and one 1
/// - Non-Resolvable Private: MSB bits = 00 (0x00-0x3F), remaining 46 bits must have at least one 0 and one 1
/// Invalid pattern like 0xa1a1a1a1a1a1 has MSB bits = 10 (not allowed)
#[derive(Clone, PartialEq, ::prost::Message)]
pub struct HostCommandConfigurePeripheral {
    /// Peripheral name
    #[prost(string, tag = "1")]
    pub name: ::prost::alloc::string::String,
    /// Peripheral addr
    ///
    /// 6 bytes for Bluetooth address
    #[prost(bytes = "vec", tag = "2")]
    pub addr: ::prost::alloc::vec::Vec<u8>,
}
/// Host command. Configure peripheral security
#[derive(Clone, Copy, PartialEq, ::prost::Message)]
pub struct HostCommandConfigurePeripheralSecurity {
    /// Passkey for pairing (6 digit numeric)
    #[prost(uint32, tag = "1")]
    pub passkey: u32,
}
/// Host command. Configure service
#[derive(Clone, Copy, PartialEq, ::prost::Message)]
pub struct HostCommandConfigureService {
    /// Service UUID (u16)
    ///
    /// u16 in Rust, using uint32 in proto3
    #[prost(uint32, tag = "1")]
    pub uuid: u32,
}
/// Host command. Configure characteristic
#[derive(Clone, PartialEq, ::prost::Message)]
pub struct HostCommandConfigureCharacteristic {
    /// Characteristic UUID (u16)
    ///
    /// u16 in Rust
    #[prost(uint32, tag = "1")]
    pub uuid: u32,
    /// Service UUID (u16) this characteristic belongs to
    ///
    /// u16 in Rust
    #[prost(uint32, tag = "2")]
    pub service_uuid: u32,
    /// Properties of the characteristic (read, write, notify, etc.)
    ///
    /// Max 4 properties per characteristic
    #[prost(enumeration = "BleProperties", repeated, tag = "3")]
    pub properties: ::prost::alloc::vec::Vec<i32>,
}
/// Host command. Configure characteristic read
#[derive(Clone, PartialEq, ::prost::Message)]
pub struct HostCommandConfigureCharacteristicRead {
    /// Characteristic UUID (u16)
    ///
    /// u16 in Rust
    #[prost(uint32, tag = "1")]
    pub uuid: u32,
    /// Service UUID (u16) this characteristic belongs to
    ///
    /// u16 in Rust
    #[prost(uint32, tag = "2")]
    pub service_uuid: u32,
    /// Read value
    ///
    /// Max MAX_NAME_SIZE bytes
    #[prost(bytes = "vec", tag = "3")]
    pub value: ::prost::alloc::vec::Vec<u8>,
}
/// Host command. Get service info
#[derive(Clone, Copy, PartialEq, ::prost::Message)]
pub struct HostCommandGetServiceInfo {
    /// Service UUID (u16)
    ///
    /// u16 in Rust
    #[prost(uint32, tag = "1")]
    pub uuid: u32,
}
/// Host command. Get characteristic info
#[derive(Clone, Copy, PartialEq, ::prost::Message)]
pub struct HostCommandGetCharacteristicInfo {
    /// Characteristic UUID (u16)
    ///
    /// u16 in Rust
    #[prost(uint32, tag = "1")]
    pub characteristic_uuid: u32,
    /// Service UUID (u16) this characteristic belongs to
    ///
    /// u16 in Rust
    #[prost(uint32, tag = "2")]
    pub service_uuid: u32,
}
/// Host command. Start advertisement. TODO: Allow selecting which services you want to advertise
#[derive(Clone, Copy, PartialEq, ::prost::Message)]
pub struct HostCommandStartAdvertisement {
    /// Allow multiple central connections
    #[prost(bool, tag = "1")]
    pub allow_multi_connect: bool,
}
/// Host command. Stop all advertisement
///
/// Empty message
#[derive(Clone, Copy, PartialEq, ::prost::Message)]
pub struct HostCommandStopAdvertisement {}
/// Host command. Notify characteristic value
#[derive(Clone, PartialEq, ::prost::Message)]
pub struct HostCommandNotifyCharacteristicValue {
    /// Device Address
    ///
    /// 6 bytes
    #[prost(bytes = "vec", tag = "1")]
    pub address: ::prost::alloc::vec::Vec<u8>,
    /// Address type
    #[prost(enumeration = "BluetoothAddressType", tag = "2")]
    pub address_type: i32,
    /// Characteristic UUID (u16)
    ///
    /// u16 in Rust
    #[prost(uint32, tag = "3")]
    pub characteristic_uuid: u32,
    /// Service UUID (u16) this characteristic belongs to
    ///
    /// u16 in Rust
    #[prost(uint32, tag = "4")]
    pub service_uuid: u32,
    /// Value to notify
    ///
    /// Max MAX_NAME_SIZE bytes
    #[prost(bytes = "vec", tag = "5")]
    pub value: ::prost::alloc::vec::Vec<u8>,
}
/// Host command. Configure BLE profile
#[derive(Clone, Copy, PartialEq, ::prost::Message)]
pub struct HostCommandConfigureProfile {
    /// Profile type to configure
    #[prost(enumeration = "BleProfile", tag = "1")]
    pub profile: i32,
}
/// Plugin data
#[derive(Clone, PartialEq, ::prost::Message)]
pub struct PluginData {
    /// Source peripheral addr that this data is originating from
    ///
    /// 6 bytes
    #[prost(bytes = "vec", tag = "1")]
    pub src_addr: ::prost::alloc::vec::Vec<u8>,
    /// Address type of the source peripheral
    #[prost(enumeration = "BluetoothAddressType", tag = "2")]
    pub src_addr_type: i32,
    /// Send type of the data
    #[prost(enumeration = "PluginDataSendType", tag = "3")]
    pub send_type: i32,
    /// Characteristic UUID (u16)
    ///
    /// u16 in Rust
    #[prost(uint32, tag = "4")]
    pub characteristic_uuid: u32,
    /// Service UUID (u16) this characteristic belongs to
    ///
    /// u16 in Rust
    #[prost(uint32, tag = "5")]
    pub service_uuid: u32,
    /// Actual command type
    #[prost(bytes = "vec", tag = "6")]
    pub data: ::prost::alloc::vec::Vec<u8>,
}
/// Service information response
#[derive(Clone, PartialEq, ::prost::Message)]
pub struct PluginServiceInfoResponse {
    /// Service UUID (u16)
    ///
    /// u16 in Rust
    #[prost(uint32, tag = "1")]
    pub service_uuid: u32,
    /// List of characteristic UUIDs (u16) in this service
    ///
    /// u16 array, max 16 characteristics per service
    #[prost(uint32, repeated, tag = "2")]
    pub characteristic_uuids: ::prost::alloc::vec::Vec<u32>,
    /// Whether the service exists
    #[prost(bool, tag = "3")]
    pub exists: bool,
}
/// Characteristic information response
#[derive(Clone, PartialEq, ::prost::Message)]
pub struct PluginCharacteristicInfoResponse {
    /// Characteristic UUID (u16)
    ///
    /// u16 in Rust
    #[prost(uint32, tag = "1")]
    pub characteristic_uuid: u32,
    /// Service UUID (u16) this characteristic belongs to
    ///
    /// u16 in Rust
    #[prost(uint32, tag = "2")]
    pub service_uuid: u32,
    /// Properties of the characteristic (read, write, notify, etc.)
    ///
    /// Max 4 properties
    #[prost(enumeration = "BleProperties", repeated, tag = "3")]
    pub properties: ::prost::alloc::vec::Vec<i32>,
    /// Whether the characteristic exists
    #[prost(bool, tag = "4")]
    pub exists: bool,
}
/// Plugin authentication completed response
#[derive(Clone, PartialEq, ::prost::Message)]
pub struct PluginAuthenticationCompletedResponse {
    /// Address of the device that was authenticated
    ///
    /// 6 bytes
    #[prost(bytes = "vec", tag = "1")]
    pub address: ::prost::alloc::vec::Vec<u8>,
    /// Address type
    #[prost(enumeration = "BluetoothAddressType", tag = "2")]
    pub address_type: i32,
    /// Whether the authentication was successful
    #[prost(bool, tag = "3")]
    pub success: bool,
}
/// Message Type IDs for protocol dispatch
/// Note: Each message type is identified by its corresponding MessageTypeId enum value
/// when serialized. The protocol uses the MessageTypeId in the message header to
/// determine which specific message type to deserialize.
/// @derive(strum::EnumIter)
#[derive(strum::EnumIter)]
#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash, PartialOrd, Ord, ::prost::Enumeration)]
#[repr(i32)]
pub enum MessageTypeId {
    /// Proto3 requires a zero value
    Unspecified = 0,
    /// Host Commands (0x01-0x0F)
    /// Configure BLE peripheral device with name and 6-byte address
    HostCommandConfigurePeripheral = 1,
    /// Create a new BLE service with specified u16 UUID
    HostCommandConfigureService = 2,
    /// Create a BLE characteristic with properties
    HostCommandConfigureCharacteristic = 3,
    /// Configure characteristic for read operations with default value
    HostCommandConfigureCharacteristicRead = 4,
    /// Query information about a BLE service
    HostCommandGetServiceInfo = 5,
    /// Query information about a BLE characteristic
    HostCommandGetCharacteristicInfo = 6,
    /// Start BLE advertising with optional multi-connect support
    HostCommandStartAdvertisement = 7,
    /// Send notification/indication to connected BLE client
    HostCommandNotifyCharacteristicValue = 8,
    /// Configure BLE security settings (pairing, bonding, passkey, etc.)
    HostCommandConfigurePeripheralSecurity = 9,
    /// Configure BLE profile with predefined services and characteristics
    HostCommandConfigureProfile = 10,
    /// Stop BLE advertising
    HostCommandStopAdvertisement = 11,
    /// Plugin Responses (0x80+)
    /// Data forwarded from BLE client to BLE plugin
    PluginData = 128,
    /// Configuration error response from plugin
    PluginConfigurationError = 129,
    /// Service information response with characteristic list
    PluginServiceInfoResponse = 130,
    /// Characteristic information response with properties
    PluginCharacteristicInfoResponse = 131,
    /// Authentication completed response from plugin
    PluginAuthenticationCompletedResponse = 132,
}
impl MessageTypeId {
    /// String value of the enum field names used in the ProtoBuf definition.
    ///
    /// The values are not transformed in any way and thus are considered stable
    /// (if the ProtoBuf definition does not change) and safe for programmatic use.
    pub fn as_str_name(&self) -> &'static str {
        match self {
            Self::Unspecified => "MESSAGE_TYPE_ID_UNSPECIFIED",
            Self::HostCommandConfigurePeripheral => "HOST_COMMAND_CONFIGURE_PERIPHERAL",
            Self::HostCommandConfigureService => "HOST_COMMAND_CONFIGURE_SERVICE",
            Self::HostCommandConfigureCharacteristic => {
                "HOST_COMMAND_CONFIGURE_CHARACTERISTIC"
            }
            Self::HostCommandConfigureCharacteristicRead => {
                "HOST_COMMAND_CONFIGURE_CHARACTERISTIC_READ"
            }
            Self::HostCommandGetServiceInfo => "HOST_COMMAND_GET_SERVICE_INFO",
            Self::HostCommandGetCharacteristicInfo => {
                "HOST_COMMAND_GET_CHARACTERISTIC_INFO"
            }
            Self::HostCommandStartAdvertisement => "HOST_COMMAND_START_ADVERTISEMENT",
            Self::HostCommandNotifyCharacteristicValue => {
                "HOST_COMMAND_NOTIFY_CHARACTERISTIC_VALUE"
            }
            Self::HostCommandConfigurePeripheralSecurity => {
                "HOST_COMMAND_CONFIGURE_PERIPHERAL_SECURITY"
            }
            Self::HostCommandConfigureProfile => "HOST_COMMAND_CONFIGURE_PROFILE",
            Self::HostCommandStopAdvertisement => "HOST_COMMAND_STOP_ADVERTISEMENT",
            Self::PluginData => "PLUGIN_DATA",
            Self::PluginConfigurationError => "PLUGIN_CONFIGURATION_ERROR",
            Self::PluginServiceInfoResponse => "PLUGIN_SERVICE_INFO_RESPONSE",
            Self::PluginCharacteristicInfoResponse => {
                "PLUGIN_CHARACTERISTIC_INFO_RESPONSE"
            }
            Self::PluginAuthenticationCompletedResponse => {
                "PLUGIN_AUTHENTICATION_COMPLETED_RESPONSE"
            }
        }
    }
    /// Creates an enum from field names used in the ProtoBuf definition.
    pub fn from_str_name(value: &str) -> ::core::option::Option<Self> {
        match value {
            "MESSAGE_TYPE_ID_UNSPECIFIED" => Some(Self::Unspecified),
            "HOST_COMMAND_CONFIGURE_PERIPHERAL" => {
                Some(Self::HostCommandConfigurePeripheral)
            }
            "HOST_COMMAND_CONFIGURE_SERVICE" => Some(Self::HostCommandConfigureService),
            "HOST_COMMAND_CONFIGURE_CHARACTERISTIC" => {
                Some(Self::HostCommandConfigureCharacteristic)
            }
            "HOST_COMMAND_CONFIGURE_CHARACTERISTIC_READ" => {
                Some(Self::HostCommandConfigureCharacteristicRead)
            }
            "HOST_COMMAND_GET_SERVICE_INFO" => Some(Self::HostCommandGetServiceInfo),
            "HOST_COMMAND_GET_CHARACTERISTIC_INFO" => {
                Some(Self::HostCommandGetCharacteristicInfo)
            }
            "HOST_COMMAND_START_ADVERTISEMENT" => {
                Some(Self::HostCommandStartAdvertisement)
            }
            "HOST_COMMAND_NOTIFY_CHARACTERISTIC_VALUE" => {
                Some(Self::HostCommandNotifyCharacteristicValue)
            }
            "HOST_COMMAND_CONFIGURE_PERIPHERAL_SECURITY" => {
                Some(Self::HostCommandConfigurePeripheralSecurity)
            }
            "HOST_COMMAND_CONFIGURE_PROFILE" => Some(Self::HostCommandConfigureProfile),
            "HOST_COMMAND_STOP_ADVERTISEMENT" => Some(Self::HostCommandStopAdvertisement),
            "PLUGIN_DATA" => Some(Self::PluginData),
            "PLUGIN_CONFIGURATION_ERROR" => Some(Self::PluginConfigurationError),
            "PLUGIN_SERVICE_INFO_RESPONSE" => Some(Self::PluginServiceInfoResponse),
            "PLUGIN_CHARACTERISTIC_INFO_RESPONSE" => {
                Some(Self::PluginCharacteristicInfoResponse)
            }
            "PLUGIN_AUTHENTICATION_COMPLETED_RESPONSE" => {
                Some(Self::PluginAuthenticationCompletedResponse)
            }
            _ => None,
        }
    }
}
/// Properties enumeration for BLE characteristics
#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash, PartialOrd, Ord, ::prost::Enumeration)]
#[repr(i32)]
pub enum BleProperties {
    /// Proto3 requires a zero value
    Unspecified = 0,
    /// Read property
    Read = 1,
    /// Write property
    Write = 2,
    /// Write without response property
    WriteNoRsp = 3,
    /// Notify property
    Notify = 4,
    /// Indicate property
    Indicate = 5,
}
impl BleProperties {
    /// String value of the enum field names used in the ProtoBuf definition.
    ///
    /// The values are not transformed in any way and thus are considered stable
    /// (if the ProtoBuf definition does not change) and safe for programmatic use.
    pub fn as_str_name(&self) -> &'static str {
        match self {
            Self::Unspecified => "BLE_PROPERTIES_UNSPECIFIED",
            Self::Read => "READ",
            Self::Write => "WRITE",
            Self::WriteNoRsp => "WRITE_NO_RSP",
            Self::Notify => "NOTIFY",
            Self::Indicate => "INDICATE",
        }
    }
    /// Creates an enum from field names used in the ProtoBuf definition.
    pub fn from_str_name(value: &str) -> ::core::option::Option<Self> {
        match value {
            "BLE_PROPERTIES_UNSPECIFIED" => Some(Self::Unspecified),
            "READ" => Some(Self::Read),
            "WRITE" => Some(Self::Write),
            "WRITE_NO_RSP" => Some(Self::WriteNoRsp),
            "NOTIFY" => Some(Self::Notify),
            "INDICATE" => Some(Self::Indicate),
            _ => None,
        }
    }
}
/// Bluetooth Device address type
#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash, PartialOrd, Ord, ::prost::Enumeration)]
#[repr(i32)]
pub enum BluetoothAddressType {
    /// Proto3 requires a zero value
    Unspecified = 0,
    /// Public address
    Public = 1,
    /// Random address
    Random = 2,
    /// Public ID address
    PublicId = 3,
    /// Random ID address
    RandomId = 4,
}
impl BluetoothAddressType {
    /// String value of the enum field names used in the ProtoBuf definition.
    ///
    /// The values are not transformed in any way and thus are considered stable
    /// (if the ProtoBuf definition does not change) and safe for programmatic use.
    pub fn as_str_name(&self) -> &'static str {
        match self {
            Self::Unspecified => "BLUETOOTH_ADDRESS_TYPE_UNSPECIFIED",
            Self::Public => "PUBLIC",
            Self::Random => "RANDOM",
            Self::PublicId => "PUBLIC_ID",
            Self::RandomId => "RANDOM_ID",
        }
    }
    /// Creates an enum from field names used in the ProtoBuf definition.
    pub fn from_str_name(value: &str) -> ::core::option::Option<Self> {
        match value {
            "BLUETOOTH_ADDRESS_TYPE_UNSPECIFIED" => Some(Self::Unspecified),
            "PUBLIC" => Some(Self::Public),
            "RANDOM" => Some(Self::Random),
            "PUBLIC_ID" => Some(Self::PublicId),
            "RANDOM_ID" => Some(Self::RandomId),
            _ => None,
        }
    }
}
/// Profile type enumeration for preconfigured BLE profiles
#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash, PartialOrd, Ord, ::prost::Enumeration)]
#[repr(i32)]
pub enum BleProfile {
    /// Proto3 requires a zero value
    Unspecified = 0,
    /// Custom profile - user-defined services
    Custom = 1,
    /// Heart Rate Monitor profile
    HeartRateMonitor = 2,
    /// Battery Service profile
    BatteryService = 3,
    /// Device Information Service profile
    DeviceInformation = 4,
}
impl BleProfile {
    /// String value of the enum field names used in the ProtoBuf definition.
    ///
    /// The values are not transformed in any way and thus are considered stable
    /// (if the ProtoBuf definition does not change) and safe for programmatic use.
    pub fn as_str_name(&self) -> &'static str {
        match self {
            Self::Unspecified => "BLE_PROFILE_UNSPECIFIED",
            Self::Custom => "CUSTOM",
            Self::HeartRateMonitor => "HEART_RATE_MONITOR",
            Self::BatteryService => "BATTERY_SERVICE",
            Self::DeviceInformation => "DEVICE_INFORMATION",
        }
    }
    /// Creates an enum from field names used in the ProtoBuf definition.
    pub fn from_str_name(value: &str) -> ::core::option::Option<Self> {
        match value {
            "BLE_PROFILE_UNSPECIFIED" => Some(Self::Unspecified),
            "CUSTOM" => Some(Self::Custom),
            "HEART_RATE_MONITOR" => Some(Self::HeartRateMonitor),
            "BATTERY_SERVICE" => Some(Self::BatteryService),
            "DEVICE_INFORMATION" => Some(Self::DeviceInformation),
            _ => None,
        }
    }
}
/// Represents the send type of the data. Was it due to a
/// write event (central -> peripheral), notify event (peripheral -> central),
/// or read attempt (central -> peripheral). Depending on which, a response
/// might be expected or sent
#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash, PartialOrd, Ord, ::prost::Enumeration)]
#[repr(i32)]
pub enum PluginDataSendType {
    /// Proto3 requires a zero value
    Unspecified = 0,
    /// Notified from the central bluetooth device
    NotifyType = 1,
    /// Read attempt from the central bluetooth device
    ReadType = 2,
    /// Written from the central bluetooth device
    WriteType = 3,
}
impl PluginDataSendType {
    /// String value of the enum field names used in the ProtoBuf definition.
    ///
    /// The values are not transformed in any way and thus are considered stable
    /// (if the ProtoBuf definition does not change) and safe for programmatic use.
    pub fn as_str_name(&self) -> &'static str {
        match self {
            Self::Unspecified => "PLUGIN_DATA_SEND_TYPE_UNSPECIFIED",
            Self::NotifyType => "NOTIFY_TYPE",
            Self::ReadType => "READ_TYPE",
            Self::WriteType => "WRITE_TYPE",
        }
    }
    /// Creates an enum from field names used in the ProtoBuf definition.
    pub fn from_str_name(value: &str) -> ::core::option::Option<Self> {
        match value {
            "PLUGIN_DATA_SEND_TYPE_UNSPECIFIED" => Some(Self::Unspecified),
            "NOTIFY_TYPE" => Some(Self::NotifyType),
            "READ_TYPE" => Some(Self::ReadType),
            "WRITE_TYPE" => Some(Self::WriteType),
            _ => None,
        }
    }
}
/// Represents the error that can occur during plugin configuration
#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash, PartialOrd, Ord, ::prost::Enumeration)]
#[repr(i32)]
pub enum PluginConfigurationError {
    /// Proto3 requires a zero value
    Unspecified = 0,
    /// The peripheral name is too long
    PeripheralNameTooLong = 1,
    /// The peripheral address is invalid
    InvalidPeripheralUuid = 2,
    /// The service UUID (u16) is invalid
    InvalidServiceUuid = 3,
    /// The characteristic UUID (u16) is invalid
    InvalidCharacteristicUuid = 4,
    /// Advertisement without proper peripheral configuration
    AdvertisementWithoutPeripheralConfiguration = 5,
    /// Service without proper peripheral configuration
    ServiceWithoutPeripheralConfiguration = 6,
    /// Characteristic without proper service configuration
    CharacteristicWithoutServiceConfiguration = 7,
}
impl PluginConfigurationError {
    /// String value of the enum field names used in the ProtoBuf definition.
    ///
    /// The values are not transformed in any way and thus are considered stable
    /// (if the ProtoBuf definition does not change) and safe for programmatic use.
    pub fn as_str_name(&self) -> &'static str {
        match self {
            Self::Unspecified => "PLUGIN_CONFIGURATION_ERROR_UNSPECIFIED",
            Self::PeripheralNameTooLong => "PERIPHERAL_NAME_TOO_LONG",
            Self::InvalidPeripheralUuid => "INVALID_PERIPHERAL_UUID",
            Self::InvalidServiceUuid => "INVALID_SERVICE_UUID",
            Self::InvalidCharacteristicUuid => "INVALID_CHARACTERISTIC_UUID",
            Self::AdvertisementWithoutPeripheralConfiguration => {
                "ADVERTISEMENT_WITHOUT_PERIPHERAL_CONFIGURATION"
            }
            Self::ServiceWithoutPeripheralConfiguration => {
                "SERVICE_WITHOUT_PERIPHERAL_CONFIGURATION"
            }
            Self::CharacteristicWithoutServiceConfiguration => {
                "CHARACTERISTIC_WITHOUT_SERVICE_CONFIGURATION"
            }
        }
    }
    /// Creates an enum from field names used in the ProtoBuf definition.
    pub fn from_str_name(value: &str) -> ::core::option::Option<Self> {
        match value {
            "PLUGIN_CONFIGURATION_ERROR_UNSPECIFIED" => Some(Self::Unspecified),
            "PERIPHERAL_NAME_TOO_LONG" => Some(Self::PeripheralNameTooLong),
            "INVALID_PERIPHERAL_UUID" => Some(Self::InvalidPeripheralUuid),
            "INVALID_SERVICE_UUID" => Some(Self::InvalidServiceUuid),
            "INVALID_CHARACTERISTIC_UUID" => Some(Self::InvalidCharacteristicUuid),
            "ADVERTISEMENT_WITHOUT_PERIPHERAL_CONFIGURATION" => {
                Some(Self::AdvertisementWithoutPeripheralConfiguration)
            }
            "SERVICE_WITHOUT_PERIPHERAL_CONFIGURATION" => {
                Some(Self::ServiceWithoutPeripheralConfiguration)
            }
            "CHARACTERISTIC_WITHOUT_SERVICE_CONFIGURATION" => {
                Some(Self::CharacteristicWithoutServiceConfiguration)
            }
            _ => None,
        }
    }
}
