...
fn test_parse_complete_protocol() -> Result<()> {
    ...
    assert!(result
        .constants
        .iter()
        .any(|c| c.name == "MESSAGE_MAGIC" && c.value == "0xDEAD"));
    assert!(result
        .constants
        .iter()
        .any(|c| c.name == "DEFAULT_PACKET_SIZE" && c.value == "256"));
...
fn test_parse_mixed_visibility_and_attributes() -> Result<()> {
    ...
    let public_enum = result
        .enums
        .iter()
        .find(|e| e.name == "PublicEnum")
        .unwrap();
...
fn test_integration_with_real_protocol_files() -> Result<()> {
    ...
    if lib_path.exists() {
        ...
        assert!(lib_result
            .constants
            .iter()
            .any(|c| c.name == "DEFAULT_PACKET_SIZE"));
    }
...
fn test_generated_python_syntax_validity() -> Result<()> {
    ...
    for constant in &result.constants {
        ...
        assert!(constant
            .value
            .chars()
            .all(|c| c.is_alphanumeric() || "0x_.\"".contains(c)));
    }
